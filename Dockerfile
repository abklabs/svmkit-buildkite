...
RUN LATEST=$(curl -s https://api.github.com/repos/alexanderguy/opsh/releases/latest | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/') && \
    git clone --depth 1 --branch "$LATEST" https://github.com/alexanderguy/opsh.git && \
    (cd opsh && make install) && rm -rf opsh
...
RUN source /arch.env && \
  if [ "$ARCH" = "amd64" ]; then \
    sh -c "$(wget -qO- https://release.anza.xyz/stable/install)"; \
    solana --version \
    else \
    echo "Skipping Anza install on $ARCH"; \
    fi
# Optional: verify tools
RUN pulumi version && go version && dotnet --version
WORKDIR /workdir
